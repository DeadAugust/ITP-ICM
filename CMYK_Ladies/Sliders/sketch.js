...
function draw() {
  ...
  //only update ellipse arrays if dotSize or Zoom changes  
  if (dSlide != lastD || zSlide != lastZ) {
    lastD = dSlide;
    lastZ = zSlide;
    Cdots = [];
    Mdots = [];
    ...
    //CK load
    for (let x = -(w2 + zSlide) / 2; x < (w2 + zSlide) / 2; x += dSlide) {
      for (let y = -(h2 + zSlide) / 2; y < (h2 + zSlide) / 2; y += dSlide) {
        let xCK = int(map(x, -(w2 + zSlide) / 2, (w2 + zSlide) / 2, 0, w2));
        let yCK = int(map(y, -(h2 + zSlide) / 2, (h2 + zSlide) / 2, 0, h2));
    ...
    for (let x = -(w2 + zSlide) / 2; x < (w2 + zSlide) / 2; x += dSlide) {
      for (let y = -(h2 + zSlide) / 2; y < (h2 + zSlide) / 2; y += dSlide) {
        let xM = int(map(x, -(w2 + zSlide) / 2, (w2 + zSlide) / 2, 0, w2));
        let yM = int(map(y, -(h2 + zSlide) / 2, (h2 + zSlide) / 2, 0, h2));
    ...
    //Y load
    for (let x = -(w + (zSlide * watio)) / 2; x < (w + (zSlide * watio)) / 2; x += dSlide) {
      for (let y = -(h + (zSlide * hatio)) / 2; y < (h + (zSlide * hatio)) / 2; y += dSlide) {
        let xY = int(map(x, -(w + (zSlide * watio)) / 2, (w + (zSlide * watio)) / 2, 0, w));
        let yY = int(map(y, -(h + (zSlide * hatio)) / 2, (h + (zSlide * hatio)) / 2, 0, h));
